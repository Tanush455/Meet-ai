    ...
    .query(async ({ input }) => {
      try {
        const response = await fetch(
          `https://newsapi.org/v2/everything?q=artificial%20intelligence%20OR%20machine%20learning%20OR%20AI%20technology&sortBy=publishedAt&language=en&pageSize=${input.limit}&apiKey=${process.env.NEWS_API_KEY}`,
          {
            headers: {
